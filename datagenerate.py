...
USER = 'iman'
PASSWORD = 'influxdb110!'
DBNAME = 'iman-randomdb'


def main(host='localhost', port=8086, nb_day=15):
    """Instantiate a connection to the backend."""
    nb_day = 7  # number of day to generate time series
    ...
    client.create_retention_policy(retention_policy, '3d', 1)

    print("Write points #: {0}".format(total_records))
    #client.write_points(series, retention_policy=retention_policy)
    #client.write_points(points=series, time_precision='s', retention_policy=retention_policy)
    client.write_points(points=series, time_precision='s')
